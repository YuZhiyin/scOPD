...
# Sparse sibling routing with offline O1 used only when a failed rollout has
# no qualified exact sibling in its K=8 group.
SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
REPO_DIR=${REPO_DIR:-/mnt/shared-storage-user/yuzhiyin/SDPO}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3-8b-nt-rsft10ep-sparse-sibling-o1-dw-k8-5ep-seed0}
JOB_NAME=${JOB_NAME:-qwen3-8b-nt-sparse-sibling-o1-dw-v1}
...
export CONFIG_NAME=${CONFIG_NAME:-nonthinking_reasoning_o1_fallback_dynamic_entropy_sibling_srpo}
export REWARD_FN_NAME=${REWARD_FN_NAME:-compute_nonthinking_reasoning_sparse_o1_fallback_sibling_score}
