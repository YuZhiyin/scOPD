...
# Correct complete rollout -> GRPO; failed complete rollout -> GT-conditioned SDPO.
# The routing decision is per rollout, while correctness is strict exact match
# over every cell in that rollout's CellPuzzles batch.
...
RAY_TMPDIR=${RAY_TMPDIR:-/tmp/ray-cell-routed-sdpo-${RUN_TS}}
...
MAX_REPROMPT_LEN=${MAX_REPROMPT_LEN:-4096}
# Match the Cell-o1 GRPO baseline so routing/distillation is the only
# optimization-method difference between the two experiments.
...
"${PYTHON_BIN}" -m verl.trainer.main_ppo --config-name routed_sdpo \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=routed_sdpo \
    ...
    trainer.group_name=gt_privileged_routed_sdpo \
...
"${RAY_CMD[@]}" stop --force 2>/dev/null || true
echo "[done] GT-routed SDPO output: ${EXP_OUT}"
