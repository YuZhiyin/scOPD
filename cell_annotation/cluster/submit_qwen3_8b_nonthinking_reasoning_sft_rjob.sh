...
RUN_SCRIPT=${RUN_SCRIPT:-${REPO_DIR}/cell_annotation/cluster/run_qwen3_8b_nonthinking_reasoning_sft_rjob.sh}
RUN_TS=${RUN_TS:-$(date +%Y%m%d_%H%M%S)}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3-8b-nonthinking-reasoning-sft-10ep-seed0}
JOB_NAME=${JOB_NAME:-qwen3-8b-nt-rsft-10ep-seed0}
PRIORITY=${PRIORITY:-5}
GPU=${GPU:-8}
CPU=${CPU:-128}
...
CMD=(
    ...
    -e DATA_ROOT="${DATA_ROOT:-${REPO_DIR}/cell_annotation/data/cello1_nonthinking_reasoning_sft_reproduction}"
