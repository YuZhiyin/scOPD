...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _update_teacher(self) -> None:
        ...
        if not self_distillation_cfg or loss_mode not in {
            "sdpo",
            "routed_sdpo",
            "joint_grpo_sdpo",
    ...
    def _prepare_dynamic_entropy_weights(
        self,
        ...
        """Precompute globally normalized DW-SDPO weights for a mini-batch.

        A no-grad teacher prepass is required because PPO micro-batching would
        otherwise normalize each rollout independently.  The paper defines
        the normalizer over all valid SDPO tokens; we additionally all-reduce
        it over the data-parallel group.
        """
        device = get_device_id()
        global_sdpo_token_count = torch.zeros((), device=device, dtype=torch.float32)
        for micro_batch in micro_batches:
            response_mask = micro_batch.batch["response_mask"]
            sdpo_mask = micro_batch.batch["self_distillation_mask"]
            token_mask = micro_batch.batch.get("distillation_token_mask")
            valid_mask = response_mask * sdpo_mask.unsqueeze(1)
            if token_mask is not None:
                valid_mask = valid_mask * token_mask
            global_sdpo_token_count += valid_mask.sum().to(
                device=device, dtype=torch.float32
            )
        if torch.distributed.is_initialized():
            torch.distributed.all_reduce(global_sdpo_token_count, op=torch.distributed.ReduceOp.SUM)

        if global_sdpo_token_count.item() == 0:
            unit_weights = [torch.ones_like(m.batch["response_mask"], dtype=torch.float32) for m in micro_batches]
            return unit_weights, {
                "dynamic_entropy/teacher_entropy_mean": 0.0,
                "dynamic_entropy/unnormalized_weight_mean": 0.0,
                "dynamic_entropy/normalized_weight_mean": 0.0,
            }

        teacher_model = self.teacher_module or self.actor_module
        raw_weights_cpu: list[torch.Tensor] = []
        local_raw_sum = torch.zeros((), device=device, dtype=torch.float32)
        local_entropy_sum = torch.zeros((), device=device, dtype=torch.float32)
        with torch.no_grad():
            for micro_batch in micro_batches:
                ...
                teacher_entropy = teacher_outputs["entropys"].float()
                valid_mask = (
                    model_inputs["response_mask"]
                    * model_inputs["self_distillation_mask"].unsqueeze(1)
                ).float()
                if "distillation_token_mask" in model_inputs:
                    valid_mask = (
                        valid_mask * model_inputs["distillation_token_mask"].float()
                    )
                raw_weight = torch.exp(-float(beta) * teacher_entropy)
                local_raw_sum += (raw_weight * valid_mask).sum()
                local_entropy_sum += (teacher_entropy * valid_mask).sum()
                raw_weights_cpu.append(raw_weight.cpu())

        if torch.distributed.is_initialized():
            torch.distributed.all_reduce(local_raw_sum, op=torch.distributed.ReduceOp.SUM)
            torch.distributed.all_reduce(local_entropy_sum, op=torch.distributed.ReduceOp.SUM)
        mean_raw_weight = local_raw_sum / global_sdpo_token_count
        normalized_weights = [
            entropy_confidence_weights(
                teacher_entropy=(-raw.clamp(min=1e-30).log() / float(beta)),
                beta=beta,
                mean_unnormalized_weight=mean_raw_weight.cpu(),
            )
            for raw in raw_weights_cpu
        ]
        return normalized_weights, {
            "dynamic_entropy/teacher_entropy_mean": (
                local_entropy_sum / global_sdpo_token_count
            ).item(),
            "dynamic_entropy/unnormalized_weight_mean": mean_raw_weight.item(),
            "dynamic_entropy/normalized_weight_mean": 1.0,
        }
    ...
    def update_policy(self, data: DataProto):
        ...
        self_distillation_enabled = loss_mode in {
            "sdpo",
            "routed_sdpo",
            "joint_grpo_sdpo",
            "selective_joint_grpo_sdpo",
        }
        routed_sdpo_enabled = loss_mode == "routed_sdpo"
        joint_grpo_sdpo_enabled = loss_mode == "joint_grpo_sdpo"
        selective_joint_grpo_sdpo_enabled = (
            loss_mode == "selective_joint_grpo_sdpo"
        )
        self_distillation_cfg = getattr(self.config, "self_distillation", None)
        if self_distillation_enabled:
            ...
            if routed_sdpo_enabled:
                self_distillation_required_keys.add("grpo_mask")
            if self_distillation_cfg.get("reasoning_only_distillation", False):
                self_distillation_required_keys.add("distillation_token_mask")
            if joint_grpo_sdpo_enabled and self_distillation_cfg.get(
                "source_aware_joint_weighting", False
            ):
                self_distillation_required_keys.add("joint_sdpo_sample_weight")
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                routed_global_token_normalization = bool(
                    routed_sdpo_enabled
                    and self_distillation_cfg.get("global_token_normalization", False)
                )
                global_routed_token_count = None
                routed_dp_size = 1
                if routed_global_token_normalization:
                    global_routed_token_count = mini_batch.batch["response_mask"].sum().to(
                        device=get_device_id(), dtype=torch.float32
                    )
                    if torch.distributed.is_initialized():
                        torch.distributed.all_reduce(
                            global_routed_token_count,
                            op=torch.distributed.ReduceOp.SUM,
                        )
                        routed_dp_size = torch.distributed.get_world_size()
                    global_routed_token_count = global_routed_token_count.clamp(min=1.0)
                ...
                dynamic_entropy_enabled = bool(
                    self_distillation_enabled
                    and self_distillation_cfg.get("dynamic_entropy_weighting", False)
                )
                dynamic_entropy_weights = None
                dynamic_entropy_metrics: dict[str, float] = {}
                if dynamic_entropy_enabled:
                    if self.config.loss_agg_mode != "token-mean":
                        raise ValueError(
                            "dynamic entropy weighting requires token-mean loss aggregation"
                        )
                    dynamic_entropy_weights, dynamic_entropy_metrics = (
                        self._prepare_dynamic_entropy_weights(
                            micro_batches=micro_batches,
                            temperature=temperature,
                            pad_token_id=pad_token_id,
                            beta=self_distillation_cfg.get("dynamic_entropy_beta", 1.0),
                        )
                    )
                ...
                for micro_batch_idx, micro_batch in enumerate(micro_batches):
                    ...
                    grpo_mask = model_inputs.get("grpo_mask") if routed_sdpo_enabled else None
                    distillation_token_mask = model_inputs.get(
                        "distillation_token_mask"
                    )
                    joint_sdpo_sample_weight = model_inputs.get(
                        "joint_sdpo_sample_weight"
                    )
                    if self_distillation_enabled:
                        assert not has_multi_modal_inputs, "Multi-modal inputs are not supported for distillation"

                    if routed_global_token_normalization:
                        # The routed numerator is explicitly normalized by the
                        # full mini-batch token count below, so micro-batches
                        # must be summed rather than averaged again.
                    ...
                    if self_distillation_enabled:
                        ...
                        distillation_token_weights = (
                            dynamic_entropy_weights[micro_batch_idx].to(
                                device=log_prob.device,
                                dtype=log_prob.dtype,
                            )
                            if dynamic_entropy_weights is not None
                            else None
                        )
                        if joint_sdpo_sample_weight is not None:
                            source_token_weights = joint_sdpo_sample_weight.to(
                                device=log_prob.device, dtype=log_prob.dtype
                            ).unsqueeze(1).expand_as(log_prob)
                            distillation_token_weights = (
                                source_token_weights
                                if distillation_token_weights is None
                                else distillation_token_weights * source_token_weights
                            )
                        ...
                        sdpo_loss, pg_metrics = compute_self_distillation_loss(
                            ...
                            distillation_token_weights=distillation_token_weights,
                            distillation_token_mask=distillation_token_mask,
                        )

                        if dynamic_entropy_metrics:
                            pg_metrics.update(dynamic_entropy_metrics)

                        pg_metrics["self_distillation/empty_target_batch"] = self_distillation_mask.sum().item() == 0
                        if routed_sdpo_enabled:
                            sdpo_response_mask = response_mask * self_distillation_mask.unsqueeze(1)
                            grpo_response_mask = response_mask * grpo_mask.unsqueeze(1)
                            sdpo_token_count = sdpo_response_mask.sum()
                            grpo_token_count = grpo_response_mask.sum()
                            total_routed_tokens = (sdpo_token_count + grpo_token_count).clamp(min=1.0)

                            if grpo_token_count.item() > 0:
                            ...
                            # helper recovers the global routed-token objective.
                            pg_loss = combine_routed_branch_losses(
                                sdpo_loss=sdpo_loss,
                                grpo_loss=grpo_loss,
                                sdpo_token_count=sdpo_token_count,
                                grpo_token_count=grpo_token_count,
                                global_token_count=(
                                    global_routed_token_count
                                    if routed_global_token_normalization
                                    else None
                                ),
                                dp_size=routed_dp_size,
                            )
                            pg_metrics.update(
                                {
                                    "routing/sdpo_branch_loss": sdpo_loss.detach().item(),
                                    "routing/grpo_branch_loss": grpo_loss.detach().item(),
                                    "routing/sdpo_token_fraction_actor": (
                                        sdpo_token_count / total_routed_tokens
                                    ).detach().item(),
                                    "routing/grpo_token_fraction_actor": (
                                        grpo_token_count / total_routed_tokens
                                    ).detach().item(),
                                    "routing/sdpo_sample_fraction_actor": (
                                        self_distillation_mask.mean().detach().item()
                                    ),
                                    "routing/grpo_sample_fraction_actor": (
                                        grpo_mask.mean().detach().item()
                                    ),
                                    "routing/grpo_pg_clipfrac": grpo_metrics[
                                        "actor/pg_clipfrac"
                                    ],
                                    "routing/grpo_ppo_kl": grpo_metrics["actor/ppo_kl"],
                                    "routing/grpo_pg_clipfrac_lower": grpo_metrics[
                                        "actor/pg_clipfrac_lower"
                                    ],
                                    "routing/grpo_nonzero_adv_sample_fraction_actor": (
                                        (
                                            advantages.abs().sum(dim=-1) > 1e-8
                                        ).float()
                                        * grpo_mask
                                    ).sum().detach().item()
                                    / grpo_mask.sum().clamp(min=1.0).detach().item(),
                                }
                            )
                        elif joint_grpo_sdpo_enabled or selective_joint_grpo_sdpo_enabled:
                            grpo_loss, grpo_metrics = get_policy_loss_fn("vanilla")(
                                old_log_prob=old_log_prob,
                                log_prob=log_prob,
                                advantages=advantages,
                            ...
                            source_aware_weighting = bool(
                                self_distillation_cfg.get(
                                    "source_aware_joint_weighting", False
                                )
                            )
                            if source_aware_weighting:
                                effective_sdpo_mask = (
                                    response_mask * self_distillation_mask.unsqueeze(1)
                                )
                                if distillation_token_mask is not None:
                                    effective_sdpo_mask = (
                                        effective_sdpo_mask * distillation_token_mask
                                    )
                                source_weight_tokens = (
                                    joint_sdpo_sample_weight.to(
                                        device=log_prob.device, dtype=log_prob.dtype
                                    ).unsqueeze(1)
                                    * effective_sdpo_mask
                                )
                                joint_weight_tensor = source_weight_tokens.sum() / (
                                    effective_sdpo_mask.sum().clamp(min=1.0)
                                )
                                pg_loss = combine_source_weighted_joint_losses(
                                    grpo_loss=grpo_loss,
                                    source_weighted_sdpo_loss=sdpo_loss,
                                    mean_sdpo_weight=joint_weight_tensor,
                                )
                                joint_weight = joint_weight_tensor.detach().item()
                            else:
                                joint_weight = float(
                                    self_distillation_cfg.get(
                                ...
                                if selective_joint_grpo_sdpo_enabled:
                                    pg_loss = combine_selective_joint_grpo_sdpo_losses(
                                        grpo_loss=grpo_loss,
                                        sdpo_loss=sdpo_loss,
                                        sdpo_weight=joint_weight,
                                    )
                                else:
                                    pg_loss = combine_joint_grpo_sdpo_losses(
                                        grpo_loss=grpo_loss,
                                        sdpo_loss=sdpo_loss,
                                        sdpo_weight=joint_weight,
                                    )
                            grpo_coefficient = (
                                1.0
                                if selective_joint_grpo_sdpo_enabled
                                else 1.0 / (1.0 + joint_weight)
                            )
                            sdpo_coefficient = (
                                joint_weight
                                if selective_joint_grpo_sdpo_enabled
                                else joint_weight / (1.0 + joint_weight)
                            )
                            pg_metrics.update(
                                {
                                    "joint/grpo_branch_loss": grpo_loss.detach().item(),
                                    "joint/sdpo_branch_loss": sdpo_loss.detach().item(),
                                    "joint/sdpo_weight": joint_weight,
                                    "joint/grpo_normalized_coefficient": grpo_coefficient,
                                    "joint/sdpo_normalized_coefficient": sdpo_coefficient,
                                    "joint/source_aware_weighting": float(
                                        source_aware_weighting
                                    ),
                                    "joint/reasoning_only_distillation": float(
                                        distillation_token_mask is not None
                                    ),
                                    "selective_joint/enabled_actor": float(
                                        selective_joint_grpo_sdpo_enabled
                                    ),
                                    "selective_joint/grpo_coverage_actor": (
                                        1.0
                                        if selective_joint_grpo_sdpo_enabled
                                        else 0.0
                                    ),
                                    "selective_joint/sdpo_sample_fraction_actor": (
                                        self_distillation_mask.mean().detach().item()
                                        if selective_joint_grpo_sdpo_enabled
                                        else 1.0
                                    ),
                                    "joint/grpo_pg_clipfrac": grpo_metrics[
                                        "actor/pg_clipfrac"
                                    ],
                                    "joint/grpo_ppo_kl": grpo_metrics["actor/ppo_kl"],
                                    "joint/grpo_pg_clipfrac_lower": grpo_metrics[
                                        "actor/pg_clipfrac_lower"
                                    ],
                                    "joint/grpo_nonzero_adv_sample_fraction": (
                                        advantages.abs().sum(dim=-1) > 1e-8
                                    )
                                    .float()
                                    .mean()
