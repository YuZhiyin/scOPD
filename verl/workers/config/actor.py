...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        Distillation is enabled when policy_loss.loss_mode is "sdpo",
        "routed_sdpo", "joint_grpo_sdpo", or
    ...
    # When true, a requested SDPO correction is taken only when a successful
    # sibling solution is constructable; otherwise the sample falls back to
    # GRPO, matching SRPO's teacher-availability routing rule.
    require_successful_solution_for_sdpo: bool = False
    # Permit environment feedback to serve as privileged teacher information
    # when no successful sibling exists. Feedback remains subordinate to a
    # sibling when environment_feedback_only_without_solution is enabled.
    allow_environment_feedback_fallback_for_sdpo: bool = False
    # Optional reward-extra key used in addition to the scalar reward threshold
    # when selecting successful sibling demonstrations.
    success_eligibility_key: Optional[str] = None
    # Normalize routed token losses over the complete PPO mini-batch (and data
    # parallel group), rather than independently within each micro-batch.
    global_token_normalization: bool = False
    # Paper-aligned Dynamic-Weighted SDPO: reweight each SDPO token by the
    # normalized confidence exp(-beta * teacher_entropy).
    dynamic_entropy_weighting: bool = False
    dynamic_entropy_beta: float = 1.0
    # Relative SDPO coefficient in
    # (L_GRPO + joint_loss_weight * L_SDPO) / (1 + joint_loss_weight).
    joint_loss_weight: float = 0.3
    # Optional per-teacher-source coefficients for all-rollout joint training.
    # When enabled, these replace joint_loss_weight for sibling, O1-fallback,
    # and GT-only teacher contexts, respectively.
    source_aware_joint_weighting: bool = False
    sibling_joint_loss_weight: float = 0.5
    o1_joint_loss_weight: float = 0.3
    gt_only_joint_loss_weight: float = 0.1
    # Restrict SDPO to the generated prefix before the answer block. GRPO still
    # trains on the entire response, including the final answer.
    reasoning_only_distillation: bool = False
    reasoning_answer_delimiter: str = "<answer>"
    # Whether verified on-policy sibling responses may be added to the
    # privileged teacher context.
    include_successful_sibling: bool = True
    # Optional always-on privilege (CellPuzzles uses the verified GT answer).
    include_base_feedback: bool = False
    base_feedback_key: str = "teacher_base_feedback"
    base_feedback_template: str = "{base_feedback_raw}"
    ...
    def __post_init__(self):
        ...
        if self.dynamic_entropy_beta <= 0.0:
            raise ValueError(
                "self_distillation.dynamic_entropy_beta must be positive, got "
                f"{self.dynamic_entropy_beta}"
            )
        if self.joint_loss_weight < 0.0:
            raise ValueError(
                "self_distillation.joint_loss_weight must be non-negative, got "
                f"{self.joint_loss_weight}"
            )
        for field_name in (
            "sibling_joint_loss_weight",
            "o1_joint_loss_weight",
            "gt_only_joint_loss_weight",
        ):
            value = getattr(self, field_name)
            if value < 0.0:
                raise ValueError(
                    f"self_distillation.{field_name} must be non-negative, got {value}"
                )
        if self.reasoning_only_distillation and not self.reasoning_answer_delimiter:
            raise ValueError(
                "self_distillation.reasoning_answer_delimiter must not be empty "
                "when reasoning_only_distillation is enabled"
            )
...
class PolicyLossConfig(BaseConfig):
    ...
    Args:
        loss_mode (str): Loss function mode. Options include 'vanilla',
            'clip-cov', 'kl-cov', 'gpg', 'sdpo', 'routed_sdpo', and
            'joint_grpo_sdpo', and 'selective_joint_grpo_sdpo'.
