...
class RayPPOTrainer:
    ...
    @staticmethod
    def _collect_reward_text(
        enabled: bool,
        key: str,
        reward_extra_infos_dict: Optional[dict[str, Any]],
        batch_size: int,
    ) -> list[Any]:
        """Collect a non-empty string reward-extra field for each rollout."""
        values: list[Any] = [None] * batch_size
        if enabled and reward_extra_infos_dict is not None:
    ...
    def _collect_feedback(
        ...
        """
        return RayPPOTrainer._collect_reward_text(
            enabled=include_environment_feedback,
            key="feedback",
    ...
    def _collect_solutions_by_uid(
        ...
        eligibility_mask: Optional[list[bool]] = None,
    ) -> dict[Any, list[int]]:
        seq_scores = reward_tensor.sum(dim=-1).detach().cpu().numpy()
        uids = batch.non_tensor_batch["uid"]
        success_by_uid: dict[Any, list[int]] = defaultdict(list)
        for idx, uid in enumerate(uids):
            eligible = eligibility_mask is None or eligibility_mask[idx]
            if seq_scores[idx] >= success_reward_threshold and eligible:
    ...
    @staticmethod
    def _resolve_routed_sdpo_routes(
        route_requests: list[bool],
        solution_strs: list[Optional[str]],
        require_successful_solution: bool,
        feedback_used: Optional[list[bool]] = None,
        allow_feedback_fallback: bool = False,
    ) -> list[float]:
        """Resolve verifier requests against per-rollout teacher availability."""
        if len(route_requests) != len(solution_strs):
            raise ValueError("route requests and sibling solutions must have equal length")
        if feedback_used is None:
            feedback_used = [False] * len(route_requests)
        if len(feedback_used) != len(route_requests):
            raise ValueError("route requests and feedback flags must have equal length")
        if require_successful_solution:
            return [
                float(
                    requested
                    and (
                        solution_strs[i] is not None
                        or (allow_feedback_fallback and feedback_used[i])
                    )
                )
                for i, requested in enumerate(route_requests)
            ]
        return [float(requested) for requested in route_requests]
    ...
    def _maybe_build_self_distillation_batch(
        ...
        if self_distillation_cfg is None or loss_mode not in {
            ...
            "routed_sdpo",
            "joint_grpo_sdpo",
            "selective_joint_grpo_sdpo",
        }:
            return None
        routed_sdpo = loss_mode == "routed_sdpo"
        joint_grpo_sdpo = loss_mode == "joint_grpo_sdpo"
        selective_joint_grpo_sdpo = loss_mode == "selective_joint_grpo_sdpo"
        selective_teacher_routing = routed_sdpo or selective_joint_grpo_sdpo
        ...
        base_feedback_list = self._collect_reward_text(
            enabled=self_distillation_cfg.get("include_base_feedback", False),
            key=self_distillation_cfg.get(
                "base_feedback_key", "teacher_base_feedback"
            ),
            reward_extra_infos_dict=reward_extra_infos_dict,
            batch_size=batch_size,
        )

        include_successful_sibling = self_distillation_cfg.get(
            "include_successful_sibling", True
        )
        success_eligibility_key = (
            self_distillation_cfg.get("success_eligibility_key", None)
            if include_successful_sibling
            else None
        )
        success_eligibility_mask: Optional[list[bool]] = None
        if success_eligibility_key:
            raw_eligibility = (
                reward_extra_infos_dict.get(success_eligibility_key, [])
                if reward_extra_infos_dict is not None
                else []
            )
            if len(raw_eligibility) != batch_size:
                raise ValueError(
                    "self-distillation sibling eligibility requires one "
                    f"reward_extra_info[{success_eligibility_key!r}] per rollout; "
                    f"got {len(raw_eligibility)} for batch size {batch_size}"
                )
            success_eligibility_mask = [bool(value) for value in raw_eligibility]
        ...
        if include_successful_sibling:
            success_by_uid = self._collect_solutions_by_uid(
                ...
                eligibility_mask=success_eligibility_mask,
            )
        else:
        ...
        def _build_teacher_message(i: int) -> list[dict]:
            ...
            has_base_feedback = base_feedback_list[i] is not None
            ...
            base_feedback_section = ""
            if has_base_feedback:
                base_feedback_section = self_distillation_cfg.get(
                    "base_feedback_template", "{base_feedback_raw}"
                ).format(base_feedback_raw=base_feedback_list[i])

            # combine solution and feedback sections
            if use_feedback or has_solution or has_base_feedback:
                reprompt_text = self_distillation_cfg.reprompt_template.format(
                    prompt=prompt_texts[i],
                    solution=solution_section,
                    feedback=feedback_section,
                    base_feedback=base_feedback_section,
                )
            else:
        ...
        # Standard SDPO distills every sample for which a privileged reprompt can
        # be built. Routed SDPO instead requires an explicit per-rollout routing
        # decision from the verifier. For CellPuzzles this flag means that the
        # complete rollout failed strict batch-level exact match.
        if selective_teacher_routing:
            raw_route = (
                reward_extra_infos_dict.get("route_to_sdpo", [])
                if reward_extra_infos_dict is not None
                else []
            )
            if len(raw_route) != batch_size:
                raise ValueError(
                    f"{loss_mode} requires one reward_extra_info['route_to_sdpo'] "
                    f"value per rollout; got {len(raw_route)} for batch size {batch_size}"
                )
            route_requests = [bool(value) for value in raw_route]
            require_solution = self_distillation_cfg.get(
                "require_successful_solution_for_sdpo", False
            )
            route_values = self._resolve_routed_sdpo_routes(
                route_requests,
                solution_strs,
                require_successful_solution=require_solution,
                feedback_used=feedback_used,
                allow_feedback_fallback=self_distillation_cfg.get(
                    "allow_environment_feedback_fallback_for_sdpo", False
                ),
            )
            missing_feedback = [
                i for i, value in enumerate(route_values)
                if value and not feedback_used[i] and solution_strs[i] is None
            ]
            if missing_feedback:
                raise ValueError(
                    f"{loss_mode} selected failed rollouts without teacher information: "
                    f"{missing_feedback[:8]}"
                )
            self_distillation_mask = torch.tensor(
                route_values, dtype=torch.float32, device=device
            )
            # Exclusive routed SDPO sends each rollout down exactly one branch.
            # Selective Joint instead applies dense GRPO to every rollout, so it
            # deliberately has no complementary GRPO mask.
            grpo_mask = 1.0 - self_distillation_mask if routed_sdpo else None
        elif joint_grpo_sdpo:
            privilege_available = [
                base_feedback_list[i] is not None
                or solution_strs[i] is not None
                or feedback_used[i]
                for i in range(batch_size)
            ]
            missing_privilege = [
                i for i, available in enumerate(privilege_available) if not available
            ]
            if missing_privilege:
                raise ValueError(
                    "joint_grpo_sdpo requires privileged teacher context for every "
                    f"rollout; missing indices: {missing_privilege[:8]}"
                )
        ...
        uid_values = list(batch.non_tensor_batch["uid"])
        uids = set(uid_values)
        ...
        num_with_base_feedback = sum(1 for f in base_feedback_list if f is not None)
        ...
        metrics = {
            ...
            "self_distillation/base_feedback_fraction": num_with_base_feedback
            / batch_size,
            "self_distillation/reprompt_sample_fraction": self_distillation_mask.float().mean().item(),
        }
        joint_sdpo_sample_weight: Optional[torch.Tensor] = None
        if joint_grpo_sdpo:
            sibling_sources = [s is not None for s in solution_strs]
            fallback_sources = [
                solution_strs[i] is None and feedback_used[i]
                for i in range(batch_size)
            ]
            base_only_sources = [
                base_feedback_list[i] is not None
                and solution_strs[i] is None
                and not feedback_used[i]
                for i in range(batch_size)
            ]
            metrics.update(
                {
                    "joint_teacher/sdpo_coverage": 1.0,
                    "joint_teacher/sibling_fraction": sum(sibling_sources)
                    / batch_size,
                    "joint_teacher/o1_fallback_fraction": sum(fallback_sources)
                    / batch_size,
                    "joint_teacher/gt_only_fraction": sum(base_only_sources)
                    / batch_size,
                }
            )
            if self_distillation_cfg.get("source_aware_joint_weighting", False):
                source_weights = {
                    "sibling": float(
                        self_distillation_cfg.get("sibling_joint_loss_weight", 0.5)
                    ),
                    "o1": float(
                        self_distillation_cfg.get("o1_joint_loss_weight", 0.3)
                    ),
                    "gt": float(
                        self_distillation_cfg.get("gt_only_joint_loss_weight", 0.1)
                    ),
                }
                if any(value < 0.0 for value in source_weights.values()):
                    raise ValueError(
                        "joint teacher source weights must all be non-negative; "
                        f"got {source_weights}"
                    )
                unresolved_sources = [
                    i
                    for i in range(batch_size)
                    if not (
                        sibling_sources[i]
                        or fallback_sources[i]
                        or base_only_sources[i]
                    )
                ]
                if unresolved_sources:
                    raise ValueError(
                        "source-aware joint SDPO could not resolve a teacher source "
                        f"for indices {unresolved_sources[:8]}"
                    )
                source_weight_values = [
                    source_weights["sibling"]
                    if sibling_sources[i]
                    else source_weights["o1"]
                    if fallback_sources[i]
                    else source_weights["gt"]
                    for i in range(batch_size)
                ]
                joint_sdpo_sample_weight = torch.tensor(
                    source_weight_values, dtype=torch.float32, device=device
                )
                metrics.update(
                    {
                        "joint_teacher/source_weight_mean": joint_sdpo_sample_weight.mean().item(),
                        "joint_teacher/sibling_weight": source_weights["sibling"],
                        "joint_teacher/o1_weight": source_weights["o1"],
                        "joint_teacher/gt_only_weight": source_weights["gt"],
                    }
                )

        distillation_token_mask: Optional[torch.Tensor] = None
        if self_distillation_cfg.get("reasoning_only_distillation", False):
            delimiter_text = self_distillation_cfg.get(
                "reasoning_answer_delimiter", "<answer>"
            )
            delimiter_token_ids = self.tokenizer.encode(
                delimiter_text, add_special_tokens=False
            )
            distillation_token_mask = core_algos.build_token_mask_before_subsequence(
                token_ids=responses,
                attention_mask=response_mask,
                delimiter_token_ids=delimiter_token_ids,
            )
            valid_response_tokens = response_mask.sum().clamp(min=1.0)
            metrics.update(
                {
                    "joint_teacher/reasoning_token_fraction": (
                        distillation_token_mask.sum() / valid_response_tokens
                    ).item(),
                    "joint_teacher/empty_reasoning_fraction": (
                        distillation_token_mask.sum(dim=-1) == 0
                    ).float().mean().item(),
                }
            )
        tensors = {
        ...
        if joint_sdpo_sample_weight is not None:
            tensors["joint_sdpo_sample_weight"] = joint_sdpo_sample_weight
        if distillation_token_mask is not None:
            tensors["distillation_token_mask"] = distillation_token_mask
        if selective_teacher_routing:
            if success_eligibility_mask is None:
                verified_success = [not requested for requested in route_requests]
            else:
                verified_success = success_eligibility_mask
            group_successes = defaultdict(list)
            for uid, successful in zip(uid_values, verified_success):
                group_successes[uid].append(bool(successful))
            all_success_groups = sum(all(values) for values in group_successes.values())
            all_failure_groups = sum(not any(values) for values in group_successes.values())
            mixed_groups = len(group_successes) - all_success_groups - all_failure_groups
            fallback_grpo = [
                requested and not bool(route_values[i])
                for i, requested in enumerate(route_requests)
            ]
            feedback_fallback = [
                bool(route_values[i])
            ...
            total_response_tokens = response_token_counts.sum().clamp(min=1.0)
            sdpo_response_tokens = (
                response_token_counts * self_distillation_mask
            ).sum()
            grpo_response_tokens = (
                (response_token_counts * grpo_mask).sum()
                if routed_sdpo
                else total_response_tokens
            )
            metrics.update(
                {
                    "routing/sdpo_sample_fraction": self_distillation_mask.mean().item(),
                    "routing/grpo_sample_fraction": (
                        grpo_mask.mean().item() if routed_sdpo else 1.0
                    ),
                    "routing/sdpo_token_fraction": (
                        sdpo_response_tokens / total_response_tokens
                    ).item(),
                    "routing/grpo_token_fraction": (
                        (grpo_response_tokens / total_response_tokens).item()
                        if routed_sdpo
                        else 1.0
                    ),
                    "routing/strict_success_fraction": sum(verified_success) / batch_size,
                    "routing/requested_sdpo_fraction": sum(route_requests) / batch_size,
                    "routing/fallback_grpo_fraction": sum(fallback_grpo) / batch_size,
                    "routing/feedback_fallback_fraction": sum(feedback_fallback) / batch_size,
                    "routing/all_success_group_fraction": all_success_groups / len(group_successes),
                    "routing/all_failure_group_fraction": all_failure_groups / len(group_successes),
                    "routing/mixed_group_fraction": mixed_groups / len(group_successes),
                    "routing/sdpo_sample_count": self_distillation_mask.sum().item(),
                    "routing/grpo_sample_count": (
                        grpo_mask.sum().item() if routed_sdpo else float(batch_size)
                    ),
                    "selective_joint/enabled": float(selective_joint_grpo_sdpo),
                    "selective_joint/grpo_coverage": (
                        1.0 if selective_joint_grpo_sdpo else 0.0
                    ),
                    "selective_joint/sdpo_coverage": (
                        self_distillation_mask.mean().item()
                        if selective_joint_grpo_sdpo
                        else 0.0
                    ),
                }
            )
            if routed_sdpo:
                tensors["grpo_mask"] = grpo_mask
