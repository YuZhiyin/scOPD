...
def combine_routed_branch_losses(
    sdpo_loss: torch.Tensor,
    grpo_loss: torch.Tensor,
    sdpo_token_count: torch.Tensor,
    grpo_token_count: torch.Tensor,
    global_token_count: Optional[torch.Tensor] = None,
    dp_size: int = 1,
) -> torch.Tensor:
    """Combine token-mean routed losses with local or global normalization.
    ...
    if global_token_count is not None:
        return numerator * dp_size / global_token_count.clamp(min=1.0)
    local_token_count = (sdpo_token_count + grpo_token_count).clamp(min=1.0)
    return numerator / local_token_count


def combine_joint_grpo_sdpo_losses(
    grpo_loss: torch.Tensor,
    sdpo_loss: torch.Tensor,
    sdpo_weight: float,
) -> torch.Tensor:
    """Return the normalized all-rollout joint GRPO/SDPO objective.

    Both branch losses are independently token-normalized over the same
    on-policy responses. Dividing by ``1 + sdpo_weight`` keeps the overall
    optimizer scale comparable across coefficient ablations.
    """
    if sdpo_weight < 0.0:
        raise ValueError(f"sdpo_weight must be non-negative, got {sdpo_weight}")
    return (grpo_loss + float(sdpo_weight) * sdpo_loss) / (
        1.0 + float(sdpo_weight)
    )


def combine_selective_joint_grpo_sdpo_losses(
    grpo_loss: torch.Tensor,
    sdpo_loss: torch.Tensor,
    sdpo_weight: float,
) -> torch.Tensor:
    """Add selective SDPO without weakening the all-rollout GRPO branch.

    Unlike all-rollout joint training, SDPO is sparse here: only answer-wrong
    samples with usable privileged context are active in ``sdpo_loss``.  The
    dense GRPO objective must therefore keep coefficient one, including in a
    micro-batch with no eligible SDPO samples.
    """
    if sdpo_weight < 0.0:
        raise ValueError(f"sdpo_weight must be non-negative, got {sdpo_weight}")
    return grpo_loss + float(sdpo_weight) * sdpo_loss
...
def combine_source_weighted_joint_losses(
    grpo_loss: torch.Tensor,
    source_weighted_sdpo_loss: torch.Tensor,
    mean_sdpo_weight: torch.Tensor,
) -> torch.Tensor:
    """Combine GRPO with an SDPO loss that already contains source weights.

    ``source_weighted_sdpo_loss`` is the token mean of
    ``lambda_source * token_weight * KL``.  Normalizing by the mean source
    weight recovers the constant-lambda joint objective when every sample has
    the same source, while allowing sibling, O1, and GT-only teachers to use
    different strengths in one PPO batch.
    """
    if mean_sdpo_weight.numel() != 1:
        raise ValueError("mean_sdpo_weight must be a scalar tensor")
    if mean_sdpo_weight.detach().item() < 0.0:
        raise ValueError(
            f"mean_sdpo_weight must be non-negative, got {mean_sdpo_weight.item()}"
        )
    return (grpo_loss + source_weighted_sdpo_loss) / (1.0 + mean_sdpo_weight)


def build_token_mask_before_subsequence(
    token_ids: torch.Tensor,
    attention_mask: torch.Tensor,
    delimiter_token_ids: list[int],
) -> torch.Tensor:
    """Mask valid response tokens before the first delimiter token sequence.

    Cell-annotation reasoning-only SDPO uses ``<answer>`` as the delimiter.
    A well-formed response therefore keeps the complete ``<reasoning>`` block
    (including its tags) and removes the answer block.  If a malformed rollout
    never emits the delimiter, all valid prefix tokens remain eligible so the
    privileged teacher can still correct a looping or unfinished rationale.
    """
    if token_ids.ndim != 2 or attention_mask.ndim != 2:
        raise ValueError("token_ids and attention_mask must both be rank-2")
    if token_ids.shape != attention_mask.shape:
        raise ValueError(
            "token_ids and attention_mask must have identical shapes; "
            f"got {token_ids.shape} and {attention_mask.shape}"
        )
    if not delimiter_token_ids:
        raise ValueError("delimiter_token_ids must not be empty")

    output = torch.zeros_like(attention_mask)
    delimiter = torch.as_tensor(
        delimiter_token_ids, dtype=token_ids.dtype, device=token_ids.device
    )
    delimiter_len = delimiter.numel()
    for row_idx in range(token_ids.shape[0]):
        valid_len = int(attention_mask[row_idx].sum().item())
        cutoff = valid_len
        if valid_len >= delimiter_len:
            windows = token_ids[row_idx, :valid_len].unfold(
                dimension=0, size=delimiter_len, step=1
            )
            matches = (windows == delimiter).all(dim=-1).nonzero(as_tuple=False)
            if matches.numel() > 0:
                cutoff = int(matches[0, 0].item())
        if cutoff > 0:
            output[row_idx, :cutoff] = attention_mask[row_idx, :cutoff]
    return output


def entropy_confidence_weights(
    teacher_entropy: torch.Tensor,
    beta: float,
    mean_unnormalized_weight: torch.Tensor,
) -> torch.Tensor:
    """Return paper-aligned DW-SDPO token weights.

    ``mean_unnormalized_weight`` is computed over all valid SDPO tokens in the
    PPO mini-batch (and DP group), so the resulting weights have global mean
    one and preserve the SDPO branch's overall scale.
    """
    if beta <= 0.0:
        raise ValueError(f"dynamic entropy beta must be positive, got {beta}")
    raw_weight = torch.exp(-float(beta) * teacher_entropy.float())
    return raw_weight / mean_unnormalized_weight.float().clamp(min=1e-12)
...
def compute_self_distillation_loss(
    ...
    distillation_token_weights: Optional[torch.Tensor] = None,
    ...
    if distillation_token_mask is not None:
        if distillation_token_mask.shape != response_mask.shape:
            raise ValueError(
                "distillation_token_mask must match response_mask; "
                f"got {distillation_token_mask.shape} and {response_mask.shape}"
            )
        loss_mask = loss_mask * distillation_token_mask
    ...
    if distillation_token_weights is not None:
        if distillation_token_weights.shape != per_token_loss.shape:
            raise ValueError(
                "distillation_token_weights must match the per-token loss shape; "
                f"got {distillation_token_weights.shape} and {per_token_loss.shape}"
            )
        per_token_loss = per_token_loss * distillation_token_weights
