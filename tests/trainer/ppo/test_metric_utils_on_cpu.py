...
class TestProcessValidationMetrics(unittest.TestCase):
    ...
    def test_process_validation_metrics_skips_optional_text_feedback(self):
        """Optional teacher feedback is payload, not a numeric metric."""
        data_sources = ["source1", "source1"]
        sample_inputs = ["prompt1", "prompt1"]
        infos_dict = {
            "score": [1.0, 0.0],
            "feedback": [None, "Ground-truth privileged information"],
        }

        result = process_validation_metrics(data_sources, sample_inputs, infos_dict, seed=42)

        self.assertAlmostEqual(result["source1"]["score"]["mean@2"], 0.5)
        self.assertNotIn("feedback", result["source1"])
